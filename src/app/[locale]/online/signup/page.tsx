...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "OnlineSignup" });
...
export default async function SignUpPage({ params }: Props) {
  ...
  const t = await getTranslations("OnlineSignup");
    ...
    <section className="relative overflow-hidden bg-gradient-to-br from-jamun-blue/5 via-white to-purple-50 min-h-[calc(100vh-4rem)] flex items-center py-16 md:py-20">
      ...
      <div className="max-w-md mx-auto px-4 w-full">
        ...
        <div className="flex justify-center">
          <SignUp
            path={`/${locale === "en" ? "" : locale + "/"}online/signup`}
            routing="path"
            signInUrl={`/${locale === "en" ? "" : locale + "/"}online/login`}
            forceRedirectUrl={`/${locale === "en" ? "" : locale + "/"}online/signup/complete-profile`}
          />
        </div>
